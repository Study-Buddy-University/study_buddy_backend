"""
GPU-Specific Test Script for AI Study Buddy
Tests GPU vs CPU mode explicitly with monitoring instructions
"""
import requests
import json
import time
...
def main():
    """Run GPU-focused API tests"""
    print(f"\n{Colors.HEADER}{Colors.BOLD}Starting GPU Test Suite{Colors.ENDC}\n")
    print(f"{Colors.WARNING}⚠️  Monitor htop in another terminal to verify GPU usage!{Colors.ENDC}\n")
    ...
    print(f"\n{Colors.BOLD}Phase 3: GPU Testing{Colors.ENDC}\n")
    print(f"{Colors.WARNING}{'='*80}{Colors.ENDC}")
    print(f"{Colors.WARNING}IMPORTANT: Monitor GPU usage in another terminal with: htop{Colors.ENDC}")
    print(f"{Colors.WARNING}AND/OR monitor backend logs with: docker compose logs -f backend{Colors.ENDC}")
    print(f"{Colors.WARNING}{'='*80}{Colors.ENDC}\n")
    
    conversation_ids = []
    if project_id:
        # Test same model with GPU ON
        print(f"\n{Colors.OKGREEN}{Colors.BOLD}TEST 1: GPU=TRUE{Colors.ENDC}")
        print(f"{Colors.OKGREEN}Expected: GPU usage spike in htop, backend logs show '🟢 GPU'{Colors.ENDC}")
        print(f"{Colors.WARNING}>>> WATCH htop NOW - GPU should spike <<<{Colors.ENDC}\n")
        
        input(f"{Colors.BOLD}Press ENTER when ready to start GPU=TRUE test...{Colors.ENDC}")
        ...
        # Test same model with GPU OFF
        print(f"\n{Colors.FAIL}{Colors.BOLD}TEST 2: GPU=FALSE{Colors.ENDC}")
        print(f"{Colors.FAIL}Expected: CPU only (no GPU spike), backend logs show '🔵 CPU'{Colors.ENDC}")
        print(f"{Colors.WARNING}>>> WATCH htop NOW - CPU only, NO GPU spike <<<{Colors.ENDC}\n")
        
        input(f"{Colors.BOLD}Press ENTER when ready to start GPU=FALSE test...{Colors.ENDC}")
